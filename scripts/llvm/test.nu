#!/usr/bin/env nu
# Test the Intel LLVM/DPC++ toolchain installation

def main [
    --quick (-q)  # Run quick smoke tests only
    --sycl-only   # Only run SYCL tests (skip LLVM lit tests)
] {
    ...
    if not ($install_prefix | path exists) {
        ...
        print "Run: pixi run install"
        exit 1
    }

    let bin_dir = $"($install_prefix)/bin"
    let clang = $"($bin_dir)/clang++"
    let sycl_ls = $"($bin_dir)/sycl-ls"

    if not ($clang | path exists) {
        print $"Error: clang++ not found at ($clang)"
        exit 1
    }

    print "="
    print "Testing Intel LLVM/DPC++ Toolchain"
    ...
    with-env $test_env {
        run-external $clang "--version"
    }

    print "\n[2/4] Checking SYCL support..."
    if ($sycl_ls | path exists) {
        with-env $test_env {
            run-external $sycl_ls
        }
    } else {
        print "Warning: sycl-ls not found"
    }
    ...
    let test_source = $"($test_dir)/sycl_test.cpp"
    let test_binary = $"($test_dir)/sycl_test"
...
'
    ...
    # Compile the test
    with-env $test_env {
        run-external $clang "-fsycl" "-o" $test_binary $test_source
    }
    ...
    # ========================================================================
    # Optional: Run LLVM lit tests
    # ========================================================================
    if (not $quick) and (not $sycl_only) and ($build_dir | path exists) {
        print "\n[Optional] Running LLVM check-sycl tests..."
        print "This may take a while..."
        
        try {
            cmake --build $build_dir --target check-sycl
        } catch {
            print "Note: check-sycl target may not be available in all configurations"
        }
    }
