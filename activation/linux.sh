...
if [ "$LINUX_BUILD_ENV_ACTIVE" != "1" ]; then
  ...
  export CXXFLAGS="--sysroot $CONDA_BUILD_SYSROOT -isystem $PREFIX/include -I $CONDA_CUDA_ROOT/include -I$CONDA_BUILD_SYSROOT/usr/include"
  export CFLAGS="--sysroot $CONDA_BUILD_SYSROOT -isystem $PREFIX/include -I $CONDA_CUDA_ROOT/include -I$CONDA_BUILD_SYSROOT/usr/include"
