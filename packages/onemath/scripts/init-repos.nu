#!/usr/bin/env nu
# Initialize git repositories for oneMath package
...
def main [] {
    ...
    print "="
    print "Initializing repositories for oneMath package"
    ...
    for repo in ($config.repos | transpose name details) {
        ...
        let repo_ref = if ($repo.details | get -o tag) != null { $repo.details.tag } else { $repo.details.branch }
        let repo_ref_type = if ($repo.details | get -o tag) != null { "Tag" } else { "Branch" }
